...
| **Shill-time window (Birdeye)** | Birdeye `defi/txs/token/seek_by_time` | Wallets that traded **around each coin's Telegram call** | Per line you give `mint  t.me-link`. The message time is read from the public link (via a CORS proxy) and only trades in `[time − hoursBefore, time + hoursAfter]` are scanned — fast and very high-signal for coordinated shillers. Private channels: type a time instead (`mint  2024-06-20T14:30`). |
